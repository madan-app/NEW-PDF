...
document.getElementById("scopeSeg").addEventListener("click", e=>{
  ...
  scope = btn.dataset.v;
  // Reset selection when switching modes — otherwise "Selected only" inherits
  // every page from "All pages" and looks like everything is already picked.
  selected = scope === "all" ? new Set([...Array(pageCount).keys()]) : new Set();
  renderThumbs();
});

function rotate(delta){
  const targets = scope==="all" ? [...Array(pageCount).keys()] : [...selected];
  if(!targets.length){ toast("Select at least one page first"); return; }
