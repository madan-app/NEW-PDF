...
async function handleFile(file){
  ...
  marked.clear();
  delete thumbGrid.dataset.dragBound; // allow re-binding drag-reorder for the newly loaded file
...
function bindThumbEvents(doc){
  ...
  // Bind drag-reorder ONCE per uploaded file — binding it on every render (as
  // renderThumbs/bindThumbEvents runs on every delete/check/drag) stacked up
  // duplicate dragstart/dragover/dragend listeners on the same container,
  // which corrupted the page order (wrong numbering, deleting more than one
  // page, "at least one page must remain" firing incorrectly).
  if(TOOL_MODE === "reorder" && !thumbGrid.dataset.dragBound){
    thumbGrid.dataset.dragBound = "1";
