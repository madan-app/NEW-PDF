...
Site name and logo initials live in **one place**: the top of `assets/js/common.js`:
...
Changing these updates the header, footer, and every tool page's `<title>`/breadcrumb automatically. Three spots are still static HTML text (by design, since they're SEO copy, not just the brand name) and need a manual edit if you rename the site: the `<title>` tag, the "Why PDFKit?" heading, and the "most-used tools on PDFKit" line — all near the top of `index.html`.

## Changelog

- Fixed: the "Select all/Select none/checkbox" and drag-and-drop reorder logic in Remove/Extract/Reorder Pages was re-binding drag listeners on every render, corrupting page order (wrong page numbers, deleting more than the selected page, false "at least one page must remain" errors). Drag listeners are now bound once per uploaded file.
- Fixed: Rotate PDF's "Selected only" scope inherited the full page selection from "All pages", making it look like nothing could be selected. It now starts empty.
- Fixed: Word to PDF rendered inside a flex-centered container, which caused html2canvas to mis-measure text and produce overlapping/garbled output. It now renders in a plain block container with explicit width, plus proper heading/list/table styling.
- Fixed: the homepage search input's placeholder text was hidden behind the search icon due to a CSS specificity collision with the generic `input[type=text]` rule.
- Fixed: the mobile hamburger menu didn't open — added click handling and a slide-down nav panel.
- Added: Privacy Policy / Terms of Service / Contact now open an in-page modal instead of linking to `#`.
- Added: left/right skyscraper ad slots (desktop-only, hidden below ~1400px viewport width).
- Moved: "All Tools" from a separate pill button into the main nav, between "Popular" and "About".
